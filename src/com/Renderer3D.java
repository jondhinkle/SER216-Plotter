...
public class Renderer3D {
	...
	public Renderer3D(int width,int height){
		setGraphicDimensions(width,height);
		
		zbuffer=new ZBuffer[getGraphicWidth()*getGraphicHeight()];
		buildNewZBuffer();
	...
	public Renderer3D(Point3D[] ds, int width, int height) {
		setGraphicDimensions(width, height);
	...
	public void buildScreen(BufferedImage buf, ZBuffer[] zbuffer2) {
		...
		backRgb=backgroundcolor.getRGB();
		...
		buf.getRaster().setDataElements( 0,0,getGraphicWidth(),getGraphicHeight(),rgb); 
		//buf.setRGB(0,0,getGraphicWidth(),getGraphicHeight(),rgb,0,getGraphicWidth());
	...
	public void setBackgroundColor(Color backgroundcolor) {
		if(backgroundcolor == null) throw new NullPointerException(); //Throw it right away the color is used
	...
	public void setAxisColor(Color axiscolor) {
		if(axiscolor == null) throw new NullPointerException(); //Throw it right away the color is used
	...
	public void setLineColor(Color linecolor) {
		if(linecolor == null) throw new NullPointerException(); //Throw it right away the color is used
		this.linecolor = linecolor;
	}
	
	public int getGraphicWidth(){return graphicwidth;}
	
	public void setGraphicDimensions(int width, int height){
	...
	public void decomposeClippedPolygonIntoZBuffer(Polygon3D p3d,ZBuffer[] zbuffer,
		...
		for(int i=0;i<triangles.length;i++){
			...
			decomposeTriangleIntoZBufferEdgeWalking( triangles[i],linecolor,zbuffer);
	...
	private void decomposeTriangleIntoZBufferEdgeWalking(Polygon3D p3d,Color color, ZBuffer[] zbuffer) {
		...
		for(int j=(int) points[middle].y;j<points[upper].y;j++){
			...
			for(int i=start;i<end;i++){
    			...
    			if(i>=getGraphicWidth() || j>= getGraphicHeight())
    				break;
		...
		if(yy1!=yy2){
			...
			if(yy2>yy1)
				...
				for (int yy = yy1; yy <= yy2; yy++) {
	    			...
	    			if(xx>=getGraphicWidth() || yy>= getGraphicHeight())
	    				break;

					int tot=getGraphicWidth()*yy+xx;
			...
			if(xx2>xx1)
				for (int xx = xx1; xx <= xx2; xx++) {
	    			...
	    			if(yy>=getGraphicHeight() || xx>= getGraphicWidth())
	    				break;
			...
			if(yy1>=getGraphicHeight() || xx1>= getGraphicWidth())
				return;

			int tot=getGraphicWidth()*yy1+xx1;
	...
	private double calcAssY(double x, double y, double z) {
			...
			return getGraphicHeight()-((z-x*cosAlfa)+y0);
	}
